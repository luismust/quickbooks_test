...
import { formatGoogleDriveUrl, getProxiedImageUrl } from "@/lib/utils"
...
export function ImageMap({ 
  ...
  const formattedSrc = useMemo(() => {
    return src ? formatGoogleDriveUrl(src) : src
  }, [src])
  ...
  const handleError = () => {
    ...
    if (retryCount >= 4) {
      ...
      if (imageRef.current) {
        ...
        imageRef.current.onerror = () => {
          toast({
            title: "Loading Error",
            description: `Could not load the image. Try with another image.`,
            variant: "destructive"
          })
          onError?.()
        }
    ...
    if (formattedSrc.includes('drive.google.com')) {
      let fileId = '';
      
      if (formattedSrc.includes('/file/d/')) {
        fileId = formattedSrc.split('/file/d/')[1].split('/')[0];
      } else if (formattedSrc.includes('id=')) {
        fileId = formattedSrc.split('id=')[1].split('&')[0];
      }
      
      if (fileId && imageRef.current) {
        console.log(`Attempt ${retryCount + 1}: Trying alternative Google Drive format...`);
        
        // Expanded list of alternative formats to try
        ...
        // Select a format based on retry count
        const formatIndex = retryCount % alternateFormats.length;
        let alternateUrl = alternateFormats[formatIndex];
        
        // Si estamos en el último intento, probar con el proxy
        if (retryCount === 3) {
          alternateUrl = getProxiedImageUrl(alternateUrl);
          console.log('Last attempt: Using proxied URL:', alternateUrl);
        } else {
          console.log(`Trying format ${formatIndex + 1}:`, alternateUrl);
        }
        
        imageRef.current.src = alternateUrl;
        return;
      }
    }
